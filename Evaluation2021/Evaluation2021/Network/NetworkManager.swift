...
class NewtworkManager: NSObject {
    ...
    class func loadVideos(of query: String, page: Int, perPage: Int, completion: @escaping  (APIResponse) -> ()) {
        
        guard var url = URL(string: API.videos.withBaseURL) as NSURL? else { return }
        url = url.append(key: APIParams.page,value: "\(page)") as NSURL
        url = url.append(key: APIParams.perPage,value: "\(perPage)") as NSURL
        url = url.append(key: APIParams.query,value: query) as NSURL
        ...
        let task = URLSession.shared.dataTask(with: request ) {(data, response, error) in
            ...
            do {
                let decoder = JSONDecoder()
                if let videos: VideoObject = try decoder.decode(VideoObject.self, from: data) {
                    let apiResponse = APIResponse(status: .ok, errorMessage: nil, data: videos)
                    completion(apiResponse)
                } else if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any] {
                    let apiResponse = APIResponse(status: .error, errorMessage: json["code"] as? String, data: nil)
                    completion(apiResponse)
